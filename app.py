...
with tab1:
    ...
    # Session state — stocke les résultats pour éviter de relancer l'API au moindre clic
    ...
    # Affichage — persiste après changement du radio button
    if st.session_state.yt_results is not None:
        df_res = st.session_state.yt_results
        ...
        with m1:
            ...
            <div class="metric-card">
                ...
                <div class="gauge-bar-bg">
                    <div style="height:12px; width:{pct}%; background:{color}; border-radius:20px;"></div>
                </div>
...
with tab2:
    ...
    if predict_btn and user_text.strip():
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.markdown('<div class="card"><div style="font-weight:600; color:#FF6B6B; margin-bottom:0.8rem;">⚠️ Mots qui poussent vers TOXIQUE</div>', unsafe_allow_html=True)
            if toxic_words:
        ...
        with c2:
            st.markdown('<div class="card"><div style="font-weight:600; color:#26C485; margin-bottom:0.8rem;">✓ Mots qui poussent vers NORMAL</div>', unsafe_allow_html=True)
            if normal_words:
